...
original_data$Hour <- hour(original_data$DATE)
original_data$Hour

# Assuming 'Sunrise' and 'Sunset' are also in datetime format
Sunrise <- as.POSIXct("2023-01-01 06:00:00", format = "%Y-%m-%d %H:%M:%S")
Sunset <- as.POSIXct("2023-01-01 18:00:00", format = "%Y-%m-%d %H:%M:%S")

# Create a new column 'Daytime' based on 'Hour'
your_data <- your_data %>%
  mutate(Daytime = ifelse(between(Hour, hour(Sunrise), hour(Sunset)), "Day", "Night"))

# Group by 'Daytime' and calculate the average for each subset
result <- your_data %>%
  group_by(Daytime) %>%
  summarize(
    AvgDewPoint = mean(HourlyDewPointTemperature, na.rm = TRUE),
    AvgWetBulb = mean(HourlyWetBulbTemperature, na.rm = TRUE)
  )
